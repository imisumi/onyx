...
glm::mat4 Camera::Matrix(float FOVdeg, float nearPlane, float farPlane)
{
	...
	// Exports the camera matrix to the Vertex Shader
	return projection * view;  // Returns the camera matrix instead of exporting it to the shader
}
...
void Camera::Inputs(GLFWwindow* window, float deltaTime)
{
	...
	// Apply speed multiplier when shift is pressed
	if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
	{
		currentSpeed *= 4.0f;  // Multiply speed by 4 when shift is held
	}
