...
namespace Utils
{
	...
	static GLenum FrameBufferTextureFormatToGL(FrameBufferTextureFormat format)
	{
		...
		MY_ASSERT(false, "Unknown FrameBufferTextureFormat!");
		return 0;
	}
...
{
	MY_ASSERT(index < m_ColorAttachments.size(), "Index out of range!");
...
void FrameBuffer::Invalidate()
{
	...
	MY_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
...
void FrameBuffer::Resize(uint32_t width, uint32_t height)
{
	...
	{
		LOG_WARN("Attempted to resize framebuffer to the same size {0}, {1}", width, height);
		return;
	}
	// LOG_INFO("Resizing framebuffer from {0}, {1} to {2}, {3}", m_Specification.Width, m_Specification.Height, width, height);
...
{
	MY_ASSERT(attachmentIndex < m_ColorAttachments.size(), "Attachment index out of range!");
